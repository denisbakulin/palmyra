...
def registration():
    ...
    if User.query.filter_by(username=username).first():
        return (
            jsonify(
                ok=False,
                error=f"Пользаватель '{username}' уже существует! Придумайте новое имя!'",
            ),
            400,
        )
    ...
    if len(password) < MIN_PASSWORD_LENGTH:
        return (
            jsonify(
                ok=False,
                error=f"Пароль слишком короткий (минимум {MIN_PASSWORD_LENGTH} символов)",
            ),
            400,
        )
    ...
    if len(username) >= 20:
        return (
            jsonify(
                ok=False,
                error=f"Имя пользователя слишком длинное (максимум {MAX_USERNAME_LENGTH} символов)",
            ),
            400,
        )
    ...
    if len(username) < 3:
        return (
            jsonify(
                ok=False,
                error=f"Имя пользователя слишком  (минимум {MIN_USERNAME_LENGTH} символов)",
            ),
            400,
        )
