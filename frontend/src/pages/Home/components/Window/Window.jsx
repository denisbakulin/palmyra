                ...
                const editUsername = async () => {
                  try {
                    ...
                    if (e?.response?.data?.error) {
                      addNotification("error", e.response.data.error);
                    }
                  }
                };
                editUsername();
          ...
          <div className="setting-option">
            ...
            >
              ...
              >
                {istatus ? (
                  <Toast
                    message="Информация обновленa!"
                    onClose={() => setIStatus(false)}
                  />
                ) : (
                  "О себе"
                )}
      ...
      <>
        <div className="window-title">
          ...
          <div className="avatar-username">
            <img
              ...
              alt="avatar"
              className="avatar"
              id="user-dialog-avatar"
            />
            <p className="username-title" id="user-dialog-name">
              {userInfo?.username}
            </p>
          </div>
        </div>
        <div className="content">
          <div className="options">
            ...
            <div
              className="window-option btn"
              onClick={() => setHomeMode("createChat")}
            >
            ...
            <p className="gradient-text" style={{ fontSize: "1.4em" }}>
              userID: {userInfo?.id}
            </p>
          </div>
        </div>
...
const GroupElement = ({
  ...
  const removeUser = async (id) => {
    const response = await api.post("chat/remove", {
      uid: id,
      cid: chatInfo.id,
    });
    ...
    <>
      ...
      <div className="content">
        {chatInfo.private && (
          <div
            className="info-area"
            style={{ backgroundColor: "rgba(255, 0, 38, 0.32)" }}
          >
            Приватный чат
          </div>
        )}
              ...
              <div
                ...
                onClick={() => {
                  const leaveFromGroup = async () => {
                    ...
                    const response = await api.post("chat/leave", {
                      cid: chatInfo.id,
                    });
                    setWMode("none");
        ...
        <div className="users">
          {chatInfo.users
            ...
            .map((user) => (
              <div
                className={`user-btn btn ${chatInfo.admin === user.id ? "admin" : ""}`}
                key={`user-${user.id}`}
                onClick={() => {
                  setWMode("user");
              ...
              >
                ...
                {user.username}
                {chatInfo.admin === user.id && (
                  <span style={{ marginLeft: "auto" }}>админ</span>
                )}
                {chatInfo.admin === userInfo.id &&
                  chatInfo.admin !== user.id && (
                    <span
        ...
        !!value.length &&
        !isAll ? (
          <p className="no-res">Ничего не найдено </p>
        ) : (
          <>
            {!!res?.length && (
              <>
        ...
        {chatInfo.id && (
          <div className="info-area" style={{ marginTop: "auto" }}>
            <p className="gradient-text">chatID ~ {chatInfo.id}</p>
          </div>
        )}
      </div>
    </>
...
const UserElement = ({
  ...
  return (
    <>
      ...
      <div className="content">
        {info.info && <div className="info-area">{info.info}</div>}
...
const SearchGroupElement = ({ setWMode, searchInfo, setUserID, setChatID }) => {
  const openChat = () => {
  ...
  return (
    <>
      ...
      <div className="content">
        {searchInfo.member ? (
          ...
          <>
            <div className="info-area">
              Участники: {searchInfo.users.length}
            </div>
            <div
              className="simple-btn btn"
              style={{ width: "100%" }}
        ...
        <div className="info-area" style={{ marginTop: "auto" }}>
          <p className="gradient-text">chatID ~ {searchInfo.chat.id}</p>
        </div>
      </div>
    </>
  );
