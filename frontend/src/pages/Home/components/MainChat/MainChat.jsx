      ...
      {chatID ? (
        <>
          <div className="chat-header">
            {isMobile && (
              <img src={back} className="back" onClick={() => setChatID(0)} />
            )}
            <p className="chat-namee btn" onClick={onClickHeader}>
              {chatInfo.name}
            </p>
          </div>
          {!initialLoaded && loading && (
            <img
              src={load}
              alt="load"
              style={{ height: "3em", margin: "0 auto" }}
            />
          )}
          <div
          ...
          >
            {loading && hasMore && (
              <img
                src={load}
                alt="load"
                style={{ height: "3em", position: "absolute", right: "50%" }}
              />
            )}
            {m?.map((msg, idx) => {
              const nextMessage = messages[idx + 1];
              const flag =
                (nextMessage && nextMessage.user.id !== msg.user.id) ||
                idx === m.length - 1;
              return (
                <Message
                  message={msg.message}
