  ...
  return (
    <>
      ...
      {mode === "search" && (
        <>
          <div id="filter">
            <div className={user} onClick={() => setSearchOption("user")}>
              Пользователи
            </div>
            <div className={group} onClick={() => setSearchOption("chat")}>
              Группы
            </div>
          </div>
                ...
                <>
                  {results.filter((a) => a.member).length > 0 && (
                    <>
                      <div
                        ...
                        onClick={() => setShowMyChats((prev) => !prev)}
                      >
                        Мои Чаты
                      </div>
                      {showMyChats &&
                        results
                          ...
                          .map((chat) => (
                            <div
                              className="search-button member btn"
                              key={`ch-${chat.id}`}
                              onClick={async () => {
                  ...
                  {results.filter((a) => !a.member).length > 0 && (
                    <>
                      <div
                        className="separator btn"
                        onClick={() => setShowNewChats((prev) => !prev)}
                      >
                        Новые Чаты
                      </div>
              ...
              {isLoad ? (
                <>
                  <img
                    src={load}
                    alt="load"
                    style={{ height: "90%", marginRight: "0.5em" }}
                  />
                  Загрузка...
                </>
              ) : (
